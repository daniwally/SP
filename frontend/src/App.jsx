        ...
        <section className="section">
          ...
          {rangoData && (
            <>
              ...
              {/* TOP 10 CATEGORÍAS - 2 cards side by side */}
              {rangoData.categorias && rangoData.categorias.length > 0 && (
                <div className="categorias-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginTop: '20px' }}>
                  {/* Card 1: Top 10 Categorías (nombre corto) */}
                  ...
                  {/* Card 2: Top 10 Categorías Path Completo */}
                  <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: '12px', padding: '20px', border: '1px solid rgba(255,255,255,0.06)' }}>
                    <h3 style={{ color: '#fff', fontSize: '1em', fontWeight: 600, margin: '0 0 16px 0' }}>Top 10 Categorías — Path Completo</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      {rangoData.categorias.map((cat, idx) => {
                        const maxCant = rangoData.categorias[0]?.cantidad || 1
                        const pct = (cat.cantidad / maxCant) * 100
                        const rankColors = ['#d946ef', '#06b6d4', '#a855f7']
                        const barColor = idx < 3 ? rankColors[idx] : 'rgba(255,255,255,0.15)'
                        const pathParts = (cat.path || cat.nombre).split(' > ')
                        return (
                          <div key={cat.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', background: 'rgba(255,255,255,0.03)', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.04)' }}>
                            <span style={{
                              width: '28px', height: '28px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center',
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '4px' }}>
                                <span style={{ color: '#fff', fontSize: '0.8em', fontWeight: 400, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginRight: '8px' }}>
                                  {pathParts.map((part, pi) => (
                                    <span key={pi}>
                                      {pi > 0 && <span style={{ color: '#666', margin: '0 4px' }}>&rsaquo;</span>}
                                      <span style={{ color: pi === pathParts.length - 1 ? '#fff' : '#999', fontWeight: pi === pathParts.length - 1 ? 500 : 400 }}>{part}</span>
                                    </span>
                                  ))}
                                </span>
                                <span style={{ color: '#ccc', fontSize: '0.8em', whiteSpace: 'nowrap', flexShrink: 0 }}>
                                  {cat.cantidad} · ${cat.total >= 1000000 ? (cat.total / 1000000).toFixed(1) + 'M' : cat.total >= 1000 ? Math.round(cat.total / 1000) + 'K' : cat.total.toLocaleString('es-AR')}
                                </span>
                              </div>
                              <div style={{ height: '4px', borderRadius: '2px', background: 'rgba(255,255,255,0.06)', overflow: 'hidden' }}>
                                <div style={{ height: '100%', width: `${pct}%`, borderRadius: '2px', background: barColor, transition: 'width 0.6s ease' }} />
